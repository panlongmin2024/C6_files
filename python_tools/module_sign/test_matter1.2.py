...
class Matter(QThread):
    ...
    def __init__(self):
        ...
        self.is_continue: bool = True

        # self.ip = os.popen('hostname -I').readline().replace('\\n', '')
        self.test_data_dict: dict = {}
        self.states = [
            State(name='ready_to_test', on_exit='enter_uart_test'),
            State(name="test_end", on_enter=['upload_data', 'create_excel']),
        ]
        self.machine: Machine = Machine(
            model=self,
            states=self.states,
            initial='before_get_permit',
        )
        self.states2 = [
            State(name="uart_test_state", on_enter=['get_uuid', 'send_hash_for_signing'], on_exit=['write_key']),
            State(name="key_written", on_exit='check_key'), ]
    ...
    def add_transition(self):
        self.machine2.add_ordered_transitions(
            states=self.states,
            conditions=['signature_check'],
        )
        ...
        self.machine.add_transition(trigger='save_data', source=['after_upload', 'after_test'],
                                    dest='after_create_excel', before='create_excel')
        self.machine.add_transition(trigger='save_data', source='after_test', dest='after_create_excel',
                                    before='create_excel')
    ...
    def run(self) -> None:
        try:
            ...
            end_time = time.time()
            self.msg_signal.emit("耗时: " + str(end_time - start_time))
    ...
    def get_send_write(self):
        self.set_port()
        self.enter_uart_test()
        uuid = self.get_uuid()
        hash_value: str = self.sha256_hash(uuid)
        signature, self.test_message = self.send_hash_for_signing(self.config_data['pi_url'], hash_value)
    ...
    def signature_check(self):
        if self.test_data_dict['signature'] is not None:
            return True
        return False
    ...
    def send_hash_for_signing(self, server_url: str, hash_value: str, ):
...
class UseMatter(UiInterface):
    ...
    def start_test(self):
        try:
            ...
            self.test_thread.machine.set_state(
                'before_get_permit', self.test_thread)
            ...
            # self.com.baudrate = self.config_data['baud']
            # self.com.stopbits = self.config_data['stop_bit']
            # self.test_thread.com = self.com
            # if self.test_thread.com.isOpen() is False:
            #     self.test_thread.com.open()
