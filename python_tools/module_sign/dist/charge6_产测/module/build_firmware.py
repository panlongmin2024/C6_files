...
class firmware(object):
    ...
    def generate_ota_image(self, ota_filename):
        ...
        if secure_boot:
            over_ride = False
            if os.path.exists(ota_file) == True:
                over_ride = True
                img_bin_file = os.path.join(self.no_sign_bin_dir, ota_filename)
            else:
        ...
        self.generate_ota_image_internal(os.path.join(self.fw_dir, \
                                                      ota_filename + '.orig'), self.orig_ota_dir,
                                         os.path.join(self.ota_dir, 'ota.xml'))

        if secure_boot:
            if over_ride == False:
                shutil.copyfile(img_bin_file, os.path.join(self.no_sign_bin_dir, ota_filename))

    def generate_app_cert_image(self, file_dir, app_original_filename, app_cert_filename, app_filename,
                                publick_key_path):
        ...
        cmd = ['python', '-B', script_firmware_path, app_original_file, app_cert_file, app_file, publick_key_path]
        # print("build cmd : %s\n" %(cmd))
        (outmsg, exit_code) = run_cmd(cmd)
        print(outmsg)
        # print(exit_code)
    ...
    def test_generate_ota_firmwares(self):
        if CFG_TEST_GENERATE_OTA_FIRMWARES_COUNT > 0:
            ...
            for i in range(CFG_TEST_GENERATE_OTA_FIRMWARES_COUNT):
                ...
                self.generate_ota_image_internal(os.path.join(test_ota_fws, ota_fw_name), ota_temp_dir_orig,
                                                 os.path.join(ota_temp_dir, 'ota.xml'))
    ...
    def image_add_secure_header(self, original_bin, img_bin):
        img_bin_file = os.path.join(self.bin_dir, img_bin)
        original_bin_file = os.path.join(self.bin_dir, original_bin)
        if os.path.exists(original_bin_file) == False:
            return

        over_ride = False
        if os.path.exists(img_bin_file) == True:
            over_ride = True
            img_bin_file = os.path.join(self.no_sign_bin_dir, img_bin)
            shutil.copyfile(original_bin_file, img_bin_file)
        else:
            img_bin_file = os.path.join(self.bin_dir, img_bin)
            shutil.copyfile(original_bin_file, img_bin_file)
        ...
        if over_ride == False:
            shutil.copyfile(img_bin_file, os.path.join(self.no_sign_bin_dir, img_bin))


def main(argv):
    global soc_name, board_name, encrypt_fw, efuse_bin, secure_boot
    ...
    try:
        ...
        if secure_boot:
            fw.image_add_secure_header('zephyr.bin', 'zephyr_no_sign.bin')
            fw.image_add_checksum('recovery.bin')
            fw.generate_app_cert_image(fw.bin_dir, 'zephyr_no_sign.bin', 'zephyr_cert.bin', 'zephyr.bin',
                                       fw.publick_key_path)

            fw.generate_raw_image(fw_prefix + '_raw.bin')
            fw.generate_ota_image('ota_no_sign.bin')
            fw.generate_app_cert_image(fw.fw_dir, 'ota_no_sign.bin', 'ota_cert.bin', 'ota.bin', fw.publick_key_path)
        else:
            fw.image_add_checksum('zephyr.bin')
            fw.image_add_checksum('recovery.bin')
